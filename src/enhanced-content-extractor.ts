...
export class EnhancedContentExtractor {
    ...
    try {
      ...
      await context.addInitScript(() => {
        ...
        window.navigator.permissions.query = async parameters => {
          const status = await originalQuery.call(
  ...
  private shouldUseBrowser(error: unknown, url: string): boolean {
    ...
    const indicators = [
      ...
      message.includes('Low quality content detected'),
      
      // Response content suggesting bot detection
      responseData.includes('Please enable JavaScript'),
      responseData.includes('captcha'),
      responseData.includes('unusual traffic'),
      responseData.includes('robot'),
